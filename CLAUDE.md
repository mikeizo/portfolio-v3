...
### Data layer
- `src/utils/mongodb.ts` is the single gateway to Mongo via Mongoose. It exposes `fetchData`, `fetchDataById`, `updateData`, `insertData`, `deleteData` — all keyed on a raw collection name.
- **Connection strategy (serverless-tuned)**: `connectToDatabase` caches the in-flight connect *promise* on `globalThis` (`__mongooseConn`) so concurrent requests share one connection per instance and the cache survives dev HMR; a failed connect clears the cache so the next request retries. Pool options are capped for Vercel serverless (`maxPoolSize: 5`, `minPoolSize: 0`, `maxIdleTimeMS: 15s`, `serverSelectionTimeoutMS: 5s`) — each function instance gets its own pool, so keep these small; the Mongoose default of 100 caused Atlas connection-limit spikes.
...
- There is **no public read API**: the former `src/pages/api/[feed]/` routes were removed (unused, unwhitelisted, and a source of Mongo connection spikes from bot traffic). All reads happen in-process during SSR.
